...
void MarkTimeThisTick(void) {
  ...
  float newTime = tp.time - g_StartTimeb.time + (tp.millitm / 1000.0) -
                  (g_StartTimeb.millitm / 1000.0);
...
float GetElapsedTime(void) { return (g_TimeLastTick); }
...
float GetExactTime(void) {
  ...
  return tp.time - g_StartTimeb.time + (tp.millitm / 1000.0) -
         (g_StartTimeb.millitm / 1000.0);
#endif
}
